...
<template>
    <div class="app w-full mx-auto mb-8 mt-16 md:mt-8 px-4">
        ...
        <div class="mt-2 flex justify-center gap-4">
            ...
            <RouterLink to="/usage" class="">usage</RouterLink>
        ...
        <h1 class="font-bold mt-8">⚪ Intro</h1>
        <p class="mt-2">
            This simple guide will walk you through the basic usage of the library.
        </p>

        <!-- Glitching <div> -->
        <h1 class="font-bold mt-8">🟣 Glitching {{ '<div>' }}</h1>
        <p class="mt-2">The most optimized way to use PowerGlitch is to first create a placeholder div</p>
        <highlightjs
            class="rounded-xl overflow-hidden mt-2"
            language="html"
            :code="`<div class='glitch'></div>`"
        />
        <p class="mt-2">ensure the div has a non-zero width/height</p>
        <highlightjs
            class="rounded-xl overflow-hidden mt-2"
            language="css"
            :code="`
...
`.trim()"
        />
        <p class="mt-2">then use PowerGlitch.glitch() to populate the div with the glitched image</p>
        <highlightjs
            class="rounded-xl overflow-hidden mt-2"
            language="js"
            :code="`
...
`.trim()"
        />
        <p class="mt-2">this takes care of animating the placeholder with the glitch effect.</p>

        <!-- Glitching <img> -->
        <h1 class="font-bold mt-8">🔵 Glitching {{ '<img>' }}</h1>
        <p class="mt-2">It is also possible to glitch images directly</p>
        <highlightjs
            class="rounded-xl overflow-hidden mt-2"
            language="html"
            :code="`<img src='https://.../image.png' class='glitch' />`"
        />
        <p class="mt-2">for images, call PowerGlitch.glitch() without specifying the imageUrl</p>
        <highlightjs
            class="rounded-xl overflow-hidden mt-2"
            language="js"
            :code="`
...
`.trim()"
        />
        <p class="mt-2">this replaces the image with its glitched version.</p>

        <!-- By reference -->
        <h1 class="font-bold mt-8">⚫ Passing reference</h1>
        <p class="mt-2">If you already have a reference to the element to glitch, you can directly pass it as the first argument</p>
        <highlightjs
            class="rounded-xl overflow-hidden mt-2"
            language="js"
            :code="`
...
`.trim()"
        ...
        <!-- Customize glitch -->
        <h1 class="font-bold mt-8">🟢 Customize glitch</h1>
        <p class="mt-2">
...
`.trim()"
        />
        <p class="mt-2">
            to find the perfect set of options for your image, <router-link to="/playground" class="underline">
                use the playground
            </router-link>
        </p>
        
        <h1 class="font-bold mt-8">🟡 API reference</h1>
        <p class="mt-2 pb-8">
